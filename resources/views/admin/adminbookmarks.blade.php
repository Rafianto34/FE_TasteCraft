...
<head>
  ...
  <title>Admin Bookmarks - TasteCraft</title>
        ...
        <table class="min-w-full text-sm text-left text-gray-500">
          <thead class="text-xs text-gray-700 uppercase bg-gray-100">
            <tr>
              ...
              <th class="px-6 py-3">Bookmark</th>
          ...
          <tbody>
            @foreach($bookmarks as $bookmark)
              <tr class="border-b">
                <td class="px-6 py-4">{{ $bookmark->user->name }}</td>
                <td class="px-6 py-4">{{ $bookmark->recipe->name }}</td>
                <td class="px-6 py-4">
                  <form action="{{ route('admin.bookmarks.delete', $bookmark->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
